...
static const Command commands[] = {{"delta", delta_command, -1},
...
static void help() {
  ...
  xd_error(
      "  -[0-9]             ZLIB compress level: 0=none, 1=fast, 6=default, "
      "9=best\n");
  xd_error(
      "  -s=BLOCK_SIZE      Sets block size (power of 2), minimum match "
      "length\n");
  xd_error(
      "                     In-core memory requirement is (FROM_LEN * 8) / "
      "BLOCK_SIZE\n");
  exit(2);
}
...
static void xd_error_func(const gchar* log_domain, GLogLevelFlags log_level,
                          const gchar* message, gpointer user_data) {
  if (!xd_error_file) xd_error_file = stderr;

  fprintf(xd_error_file, "%s: %s", program_name, message);
}

static gboolean event_devel(void) {
...
static gboolean event_watch(GenericEvent* ev, GenericEventDef* def,
                            const char* message) {
  if (quiet && def->level <= EL_Warning) return TRUE;
...
gint main(gint argc, gchar** argv) {
   ...
   */
  {
    ...
    p = strrchr(strip_ext, '.');
    if ((p != NULL) && (strncasecmp(p + 1, "exe", 3) == 0)) *p = '\0';
  ...
  g_log_set_handler(G_LOG_DOMAIN, G_LOG_LEVEL_WARNING, xd_error_func, NULL);

  if (argc < 2) usage();

  for (cmd = commands; cmd->name; cmd += 1)
    if (strcmp(cmd->name, argv[1]) == 0) break;
  ...
  while ((c = getopt_long(argc, argv, "+nqphvVs:m:0123456789", long_options,
                          &longind)) != EOF) {
    switch (c) {
      case 'q':
        quiet = TRUE;
...
static gssize xd_handle_map_page(XdFileHandle* fh, guint pgno,
                                 const guint8** mem);
static gboolean xd_handle_unmap_page(XdFileHandle* fh, guint pgno,
                                     const guint8** mem);

static gboolean xd_fwrite(XdFileHandle* fh, const void* buf, gint nbyte) {
  return fwrite(buf, nbyte, 1, fh->out) == 1;
}

static gboolean xd_fread(XdFileHandle* fh, void* buf, gint nbyte) {
  return fread(buf, nbyte, 1, fh->in) == 1;
}

static gboolean xd_fclose(XdFileHandle* fh) { return fclose(fh->out) == 0; }

static gboolean xd_frclose(XdFileHandle* fh) { return fclose(fh->in) == 0; }

static gboolean xd_gzwrite(XdFileHandle* fh, const void* buf, gint nbyte) {
  return gzwrite(fh->out, (void*)buf, nbyte) == nbyte;
}

static gboolean xd_gzread(XdFileHandle* fh, void* buf, gint nbyte) {
  return gzread(fh->in, buf, nbyte) == nbyte;
}

static gboolean xd_gzclose(XdFileHandle* fh) {
  return gzclose(fh->out) == Z_OK;
}

static gboolean xd_gzrclose(XdFileHandle* fh) {
  return gzclose(fh->in) == Z_OK;
}

static void init_table(XdFileHandle* fh) {
...
static XdFileHandle* open_common(const char* name, const char* real_name) {
  XdFileHandle* fh;
...
static gboolean file_gzipped(const char* name, gboolean* is_compressed) {
  ...
  if (fread(buf, 2, 1, f) != 1) return TRUE;

#define GZIP_MAGIC1 037
#define GZIP_MAGIC2 0213

  if (buf[0] == GZIP_MAGIC1 && buf[1] == GZIP_MAGIC2) (*is_compressed) = TRUE;

  return TRUE;
}

static const char* xd_tmpname(void) {
...
static const char* file_gunzip(const char* name) {
...
static XdFileHandle* open_read_noseek_handle(const char* name,
                                             gboolean* is_compressed,
                                             gboolean will_read,
                                             gboolean honor_pristine) {
  XdFileHandle* fh;
  ...
  if (honor_pristine && pristine)
    *is_compressed = FALSE;
    ...
    if (!file_gzipped(name, is_compressed)) return NULL;
  }

  if ((*is_compressed) && !(name = file_gunzip(name))) return NULL;

  if (!(fh = open_common(name, name0))) return NULL;

  fh->type = READ_NOSEEK_TYPE;

  edsio_md5_init(&fh->ctx);

  if (*is_compressed) fh->cleanup = name;
...
static void xd_read_close(XdFileHandle* fh) {
  /*
  ...
  if (fh->cleanup) unlink(fh->cleanup);

  if (fh->in) (*fh->in_close)(fh);
}

static XdFileHandle* open_read_seek_handle(const char* name,
                                           gboolean* is_compressed,
                                           gboolean honor_pristine) {
  XdFileHandle* fh;
  const char* name0 = name;
...
static XdFileHandle* open_write_handle(int fd, const char* name) {
...
static gint xd_begin_compression(XdFileHandle* fh) {
  gint filepos, nfd;

  if (compress_level == 0) return fh->real_length;
...
static gboolean xd_end_compression(XdFileHandle* fh) {
  if (compress_level == 0) return TRUE;
...
static gssize xd_handle_length(XdFileHandle* fh) {
  if (fh->in_compressed)
...
static gssize xd_handle_pages(XdFileHandle* fh) {
  g_assert(fh->type & READ_TYPE);
  return xd_handle_length(fh) / XD_PAGE_SIZE;
}

static gssize xd_handle_pagesize(XdFileHandle* fh) {
  g_assert(fh->type & READ_TYPE);
  return XD_PAGE_SIZE;
}

static gint on_page(XdFileHandle* fh, guint pgno) {
  if (pgno > xd_handle_pages(fh)) return -1;

  if (pgno == xd_handle_pages(fh)) return xd_handle_length(fh) % XD_PAGE_SIZE;

  return XD_PAGE_SIZE;
}

static gboolean xd_handle_close(XdFileHandle* fh, gint ignore) {
...
static const guint8* xd_handle_checksum_md5(XdFileHandle* fh) {
  if (fh->in && !fh->md5_good) {
    ...
    while (fh->md5_page <= xd_handle_pages(fh)) {
      ...
      if ((onpage = xd_handle_map_page(fh, pgno, &page)) < 0) return NULL;
      ...
      if (!xd_handle_unmap_page(fh, pgno, &page)) return NULL;
    }
  }
...
static gboolean xd_handle_set_pos(XdFileHandle* fh, guint pos) {
  if (fh->current_pos == pos + fh->narrow_low) return TRUE;
...
static gboolean xd_handle_narrow(XdFileHandle* fh, guint low, guint high,
                                 gboolean compressed) {
  if (high > fh->length) {
  ...
  if (compressed) {
    ...
    if (!xd_handle_set_pos(fh, 0)) return FALSE;
  }

  return TRUE;
}

static guint xd_handle_get_pos(XdFileHandle* fh) {
  return fh->current_pos - fh->narrow_low;
}

static const gchar* xd_handle_name(XdFileHandle* fh) {
  return g_strdup(fh->name);
}

static gssize xd_handle_read(XdFileHandle* fh, guint8* buf, gsize nbyte) {
  if (nbyte == 0) return 0;
  ...
  if (!no_verify) edsio_md5_update(&fh->ctx, buf, nbyte);
...
static gboolean xd_handle_write(XdFileHandle* fh, const guint8* buf,
                                gsize nbyte) {
...
static gboolean xd_handle_really_close(XdFileHandle* fh) {
  g_assert(fh->type == WRITE_TYPE);
...
static LRU* pull_lru(XdFileHandle* fh, LRU* lru) {
...
static gboolean really_free_one_page(XdFileHandle* fh) {
  LRU* lru = fh->lru_tail;
...
static gboolean make_lru_room(XdFileHandle* fh) {
  if (fh->lru_count == max_mapped_pages) {
    if (!really_free_one_page(fh)) return FALSE;
  }
...
static gssize xd_handle_map_page(XdFileHandle* fh, guint pgno,
  ...
  if (fh->lru_table->len < (pgno + 1)) {
    ...
    while (olen <= pgno) fh->lru_table->pdata[olen++] = NULL;
  }
  ...
  if (lru->prev) lru->prev->next = lru;

  if (!fh->lru_tail) fh->lru_tail = lru;
  ...
  if (!lru->buffer) {
...
#else
      if (!(lru->buffer = mmap(NULL, to_map, PROT_READ, MAP_PRIVATE, fh->fd,
                               pgno * XD_PAGE_SIZE))) {
    ...
    if (pgno == fh->md5_page) {
      if (!no_verify) edsio_md5_update(&fh->ctx, lru->buffer, to_map);
      fh->md5_page += 1;
...
static gboolean xd_handle_copy(XdFileHandle* from, XdFileHandle* to, guint off,
                               guint len) {
  if (from->in) {
    guint8 buf[1024];
    ...
    while (len > 0) {
      ...
      if (xd_handle_read(from, buf, r) != r) return FALSE;

      if (!xd_handle_write(to, buf, r)) return FALSE;
    ...
    while (len > 0) {
      ...
      if (from->copy_pgno != off_page && from->copy_page &&
      ...
      if (!xd_handle_write(to, from->copy_page + off_off, copy)) return FALSE;

      if (!xd_handle_unmap_page(from, off_page, &from->copy_page)) return FALSE;
...
static gboolean xd_handle_putui(XdFileHandle* fh, guint32 i) {
  guint32 hi = g_htonl(i);

  return xd_handle_write(fh, (guint8*)&hi, 4);
}

static gboolean xd_handle_getui(XdFileHandle* fh, guint32* i) {
  if (xd_handle_read(fh, (guint8*)i, 4) != 4) return FALSE;
...
static HandleFuncTable xd_handle_table = {
    ...
    (gssize(*)(FileHandle * fh, guint pgno, const guint8** mem))
        xd_handle_map_page,
    (gboolean(*)(FileHandle * fh, guint pgno, const guint8** mem))
        xd_handle_unmap_page,
    (const guint8* (*)(FileHandle* fh))xd_handle_checksum_md5,

    (gboolean(*)(FileHandle * fh, gint flags)) xd_handle_close,

    (gboolean(*)(FileHandle * fh, const guint8* buf, gsize nbyte))
        xd_handle_write,
    (gboolean(*)(FileHandle * from, FileHandle* to, guint off, guint len))
        xd_handle_copy,
...
static void htonl_array(guint32* array, gint len) {
  gint i;

  for (i = 0; i < len; i += 1) array[i] = g_htonl(array[i]);
}

static void ntohl_array(guint32* array, gint len) {
  gint i;

  for (i = 0; i < len; i += 1) array[i] = g_ntohl(array[i]);
}
...
static gint delta_command(gint argc, gchar** argv) {
  ...
  if (!(out = open_write_handle(patch_out_fd, patch_out_name))) return 2;

  if (!(gen = xdp_generator_new())) return 2;
  ...
  if (!xd_handle_write(out, XDELTA_PREFIX, XDELTA_PREFIX_LEN)) return 2;
  ...
  if (!xd_handle_write(out, (guint8*)header_space, HEADER_SPACE)) return 2;

  if (!xd_handle_write(out, from_name, strlen(from_name))) return 2;

  if (!xd_handle_write(out, to_name, strlen(to_name))) return 2;

  if (!xd_handle_close(out, 0)) return 2;

  if ((header_offset = xd_begin_compression(out)) < 0) return 2;

  if (!(cont =
            xdp_generate_delta(gen, (FileHandle*)to, NULL, (FileHandle*)out)))
    return 2;
  ...
  if (cont->has_data && cont->has_data == cont->source_info_len) {
    if (!quiet)
      xd_error(
          "warning: no matches found in from file, patch will apply without "
          "it\n");
  }

  if (!xd_handle_close(out, 0)) return 2;

  if ((control_offset = xd_begin_compression(out)) < 0) return 2;

  if (!xdp_control_write(cont, (FileHandle*)out)) return 2;

  if (!xd_end_compression(out)) return 2;

  if (!xd_handle_putui(out, control_offset)) return 2;
  ...
  if (!xd_handle_really_close(out)) return 2;
...
static XdeltaPatch* process_patch(const char* name) {
  XdeltaPatch* patch;
  guint total_trailer;
  ...
  if (!(patch->patch_in = open_read_noseek_handle(
            name, &patch->patch_is_compressed, TRUE, TRUE)))
    return NULL;

  if (xd_handle_read(patch->patch_in, patch->magic_buf, XDELTA_PREFIX_LEN) !=
      XDELTA_PREFIX_LEN)
    return NULL;

  if (xd_handle_read(patch->patch_in, (guint8*)patch->header_space,
  ...
  } else if (strncmp(patch->magic_buf, XDELTA_104_PREFIX, XDELTA_PREFIX_LEN) ==
             0) {
    patch->has_trailer = TRUE;
  ...
  if (xd_handle_read(patch->patch_in, patch->from_name, patch->from_name_len) !=
      patch->from_name_len)
    return NULL;

  if (xd_handle_read(patch->patch_in, patch->to_name, patch->to_name_len) !=
      patch->to_name_len)
    return NULL;
  ...
  if (!xd_handle_set_pos(patch->patch_in,
                         xd_handle_length(patch->patch_in) - total_trailer))
    return NULL;

  if (!xd_handle_getui(patch->patch_in, &patch->control_offset)) return NULL;

  if (patch->has_trailer) {
    guint8 trailer_buf[XDELTA_PREFIX_LEN];

    if (xd_handle_read(patch->patch_in, trailer_buf, XDELTA_PREFIX_LEN) !=
        XDELTA_PREFIX_LEN)
      return NULL;
  ...
  if (!xd_handle_narrow(patch->patch_in, patch->header_offset,
...
static gint info_command(gint argc, gchar** argv) {
  ...
  if (!(patch = process_patch(argv[0]))) return 2;

  xd_error_file = stdout;

  xd_error("version %s found patch version %s in %s%s\n", xdelta_version,
           patch->patch_version, patch->patch_name,
  ...
  for (i = 0; i < patch->cont->source_info_len; i += 1) {
    ...
    xd_error("%s\t%d\t%d\t%d\t%s\t%s\n", buf, si->len, si->copies,
             si->copy_length, si->sequential ? "yes" : "no", si->name);
  }
...
static gint patch_command(gint argc, gchar** argv) {
  ...
  if (argc > 2)
    ...
    while ((ret = stat(patch->to_name, &sbuf)) == 0) {
      if (verbose) xd_error("to file exists: %s\n", patch->to_name);
  ...
  if (strcmp(patch->to_name, "-") == 0) {
    ...
    to_out_fd =
        open(patch->to_name, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, 0666);
  ...
  if (patch->from_source) {
    ...
    if (!(from_in = open_read_seek_handle(patch->from_name, &from_is_compressed,
                                          TRUE)))
      return 2;

    if (from_is_compressed !=
        ((patch->patch_flags & FLAG_FROM_COMPRESSED) && 1))
  ...
  if (!xdp_apply_delta(patch->cont, (FileHandle*)to_out)) return 2;

  if (patch->from_source) xd_read_close((XdFileHandle*)patch->from_source->in);

  xd_read_close(patch->patch_in);

  if (!xd_handle_really_close(to_out)) return 2;
