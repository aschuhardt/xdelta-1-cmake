...
static gboolean xdp_copy_delta_region(XdeltaControl *cont,
...
gboolean xdp_apply_delta(XdeltaControl *cont, XdeltaOutStream *res) {
  if (!xdp_copy_delta_region(cont, res)) return FALSE;

  if (!handle_close(res, 0)) return FALSE;
